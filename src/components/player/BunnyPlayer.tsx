...
export function BunnyPlayer({
  ...
  // ---------- SETUP VIDEO ----------
  useEffect(() => {
    if (stype === "bunny-iframe" || stype === "iframe" || useIframeFallback) return;
    ...
    const bindEvents = () => {
      ...
      const onPlayingEvt = () => setBuf(false);
      const onErr = () => { setLoading(false); setError("Erro ao carregar vídeo."); };
      const onProg = () => { if (v.buffered.length > 0) setBufferedEnd(v.buffered.end(v.buffered.length - 1)); };
    ...
    if (stype === "hls" && Hls.isSupported()) {
      ...
      h.on(Hls.Events.MANIFEST_PARSED, () => {
        cleanup = bindEvents();
        if (autoPlay) v.play().catch(() => { v.muted = true; setMuted(true); v.play().catch(() => {}); });
      });
      ...
      h.on(Hls.Events.ERROR, (_event, data) => {
        ...
        if (data.fatal) {
          switch (data.type) {
            ...
            default:
              setLoading(false);
              setError("Erro de streaming. Tentando fallback...");
              // Fallback para iframe se for BunnyCDN
              if (clean.includes("bunny") || clean.includes("mediadelivery")) {
  ...
  }, [src, stype, autoPlay, startTime, poster, useIframeFallback]);
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      switch (e.key) {
        ...
        case 'm': case 'M':
          e.preventDefault();
          v.muted = !v.muted; setMuted(v.muted); resetCtrl(); break;
        case 'ArrowUp':
          e.preventDefault();
          { const nv = Math.min(1, (v.volume || 0) + 0.1);
            v.volume = nv; v.muted = nv === 0;
            setVolume(nv); setMuted(nv === 0); resetCtrl(); }
          break;
        case 'ArrowDown':
          e.preventDefault();
  ...
  // ---------- IFRAME FALLBACK ----------
  if (stype === "bunny-iframe" || stype === "iframe" || useIframeFallback) {
    const embedUrl = useIframeFallback ? bunnyEmbed(src) : (stype === "iframe" ? src : bunnyEmbed(src));
    return (
      ...
      <div ref={containerRef} className={`relative w-full bg-black flex items-center justify-center ${className}`} style={{ aspectRatio: '16/9' }}>
        <div className="text-center p-8 max-w-md">
          ...
          <div className="mt-4 flex gap-2 justify-center">
            <button onClick={() => setUseIframeFallback(true)} className="rounded-full bg-red-600 px-5 py-2 text-sm font-semibold text-white hover:bg-red-700 transition">
              Usar player embed
            </button>
          </div>
        </div>
  ...
  // ---------- NATIVE VIDEO ----------
  return (
    <div ref={containerRef} className={`group relative w-full bg-black select-none overflow-hidden ${className}`} style={{ aspectRatio: '16/9' }}
      onMouseMove={resetCtrl} onMouseLeave={() => playing && setShowCtrl(false)} onClick={togglePlay}>

      <video ref={videoRef} playsInline preload="auto" crossOrigin="anonymous"
      ...
      <div className={`absolute top-0 left-0 right-0 z-30 transition-opacity duration-500 ${showCtrl ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
        <div className="bg-gradient-to-b from-black/80 via-black/30 to-transparent px-4 pt-3 pb-10">
          {title && <h2 className="text-sm sm:text-base font-semibold text-white/90 truncate max-w-[80%] drop-shadow-lg">{title}</h2>}
        </div>
      </div>
      ...
      <div className={`absolute inset-x-0 bottom-0 z-30 transition-all duration-500 ${showCtrl ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8 pointer-events-none'}`}
        ...
        <div className="relative px-3 sm:px-5 pb-3 sm:pb-4 pt-12">
          ...
          <div className="flex items-center gap-1 sm:gap-2">
            ...
            <div className="relative flex items-center" onMouseEnter={() => setHoverVol(true)} onMouseLeave={() => setHoverVol(false)}>
            ...
            {title && <span className="hidden md:block text-xs text-white/60 truncate max-w-[200px] mr-2">{title}</span>}
            {rate !== 1 && <span className="rounded bg-red-600/80 px-1.5 py-0.5 text-[10px] font-bold text-white">{rate}x</span>}
