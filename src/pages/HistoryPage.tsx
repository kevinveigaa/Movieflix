...
export function HistoryPage() {
  ...
  if (!user) {
    return (
      <div className="container-app flex min-h-[50vh] flex-col items-center justify-center gap-4 text-center">
        <p className="text-ink-400">Faa login para ver seu histrico.</p>
  ...
  return (
    <div className="container-app py-8">
      ...
      <p className="mt-1 text-sm text-ink-400">Tudo o que voc assistiu recentemente.</p>
      ...
      {history.isLoading ? (
        ...
        <div className="mt-12 flex flex-col items-center justify-center gap-4 text-center">
          ...
          <p className="text-ink-400">Seu histrico est vazio.</p>
            ...
            <div key={h.id} className="flex items-center gap-4 py-4">
              ...
              <div className="flex-1">
                ...
                <p className="mt-0.5 text-xs text-ink-400">
                  {h.media_type === 'tv' ? 'Srie' : 'Filme'}  Atualizado em {new Date(h.updated_at).toLocaleDateString('pt-BR')}
                </p>
              </div>
              <button
                onClick={() => remove.mutate(h.id)}
                className="rounded-full p-2 text-ink-400 transition hover:bg-red-600/20 hover:text-red-400"
                aria-label="Remover do histrico"
              >
