...
export function PlayerPage() {
  ...
  const onTime = useCallback((t: number) => { posRef.current = t; if (Math.abs(t - lastSavedRef.current) >= 10) saveHistory(t); }, [saveHistory]);
  const onPause = useCallback(() => saveHistory(posRef.current), [saveHistory]);
  const onEnded = useCallback(() => saveHistory(durRef.current), [saveHistory]);
  const onReady = useCallback((d: number) => { durRef.current = d; }, []);
  ...
  return (
    <div className="fixed inset-0 z-0 h-screen w-screen overflow-hidden bg-black">
      ...
      {loadingVideo ? (
        ...
        <>
          <VideoPlayer src={videoUrl} startTime={showResume ? 0 : resumePos} poster={movie?.backdrop_url ?? movie?.poster_url ?? undefined} onTimeUpdate={onTime} onReady={onReady} onPause={onPause} onEnded={onEnded} />
          {showResume && (
